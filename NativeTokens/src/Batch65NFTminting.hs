...
module Batch65NFTminting where
...
nftMintingPolicy oref tname () sContext = traceIfFalse "UTxO not consumed" hasUTxO                &&
                                          traceIfFalse "There can ONLY BE ONE!" checkMintedAmount
      where
        info :: TxInfo
...
data Collection = COL [TokenName]

--[TokenName] = [ynft1,ynft2,ynft3]
-- {-# INLINABLE nftHomeworkPolicy  #-}
-- nftHomeworkPolicy :: TxOutRef ->  Collection -> ScriptContext -> Bool
-- nftHomeworkPolicy oref tname () sContext = traceIfFalse "UTxO not consumed" hasUTxO                &&
--                                           traceIfFalse "There can ONLY BE ONE!" checkMintedAmount
--   where
--     info :: TxInfo
--     info = scriptContextTxInfo sContext
...
--change-address $address \mkSchemaDefinitions ''NFTSchema
-- mkKnownCurrencies []

--mint="$tokenamount $policyid.$tokenname" \
--minting-script-file $script \
--metadata-json-file metadata.json  \
--invalid-hereafter $slotnumber \cardano-cli transaction build \
$era \
--tx-in $utxoin \
--tx-out $address+$output+"$tokenamount $policyid.$tokenname" \
--change-address $address \mkSchemaDefinitions ''NFTSchema
